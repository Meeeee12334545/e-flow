...
# Scraper settings
SCRAPER_TIMEOUT = int(os.getenv("SCRAPER_TIMEOUT", "15000"))  # milliseconds
SCRAPER_HEADLESS = os.getenv("SCRAPER_HEADLESS", "true").lower() in ("1", "true", "yes")
SCRAPER_WAIT_AFTER_LOAD = int(os.getenv("SCRAPER_WAIT_AFTER_LOAD", "3000"))  # milliseconds

# Monitor settings
# Use env vars to control behavior in different deployments (Docker, local, etc.)
...
STORE_ALL_READINGS = os.getenv("STORE_ALL_READINGS", "true").lower() in ("1", "true", "yes")
